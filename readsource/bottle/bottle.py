...
class Router(object):
    ...
    def add(self, rule, method, target, name=None):
        ...
        filters   = []   # Lists of wildcard input filters 存放(变量名，类型转换)
        builder   = []   # Data structure for the URL builder 存放(变量名，类型转换)或者(None,静态path)
        is_static = True

        for key, filtr, conf in self._itertokens(rule):

            if filtr:
                is_static = False
                if filtr == 'default': # 有变量名没filter的情况
                    filtr = self.default_filter  # 如果没有filter，就设定filtr为're'
                mask, in_filter, out_filter = self.filters[filtr](conf)  # mask代表int/float等的对应正则匹配形式，in代表类型int/float，out是个匿名函数
                if not key: # 变量名没有，很少见吧
                    pattern += '(?:%s)' % mask
                    key = 'anon%d' % anons
                    anons += 1
                else:
                    pattern += '(?P<%s>%s)' % (key, mask) # 最终要生成一个pattern对象用于匹配请求的url
                    keys.append(key) # keys存放变量名列表
                if in_filter: filters.append((key, in_filter)) # 存放(变量名,类型)对
                builder.append((key, out_filter or str)) #　存放(变量名，类型转换)
            elif key: # 这里key是个静态path
                pattern += re.escape(key)  # 最终要生成一个pattern对象用于匹配请求的url
                builder.append((None, key)) # 作为解析结果，存放静态path

        self.builder[rule] = builder
        if name: self.builder[name] = builder # url别名

        if is_static and not self.strict_order: # 后面的默认为true
        ...
        flatpat = _re_flatten(pattern)  # 把捕获组转换为非捕获组
    ...
    # 这个函数的目的是将同属于某一method(GET/POST)的所有path的无捕获组正则用|连起来
    ...
    def _handle(self, environ):
        ...
        try:
            ...
            try:
                ...
                #import pdb;pdb.set_trace()
                return route.call(**args)  # 调用greet函数
            finally:
